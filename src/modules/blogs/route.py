...
def create_blog(request: schema.Blog, db : Session = Depends(get_db), current_user: schema.User = Depends(jwt_bearer)):
    return createBlog(request, db)


# Get all blogs data
@router.post('/blogs', status_code=status.HTTP_202_ACCEPTED)
def all_blogs(request: schema.SearchBlog, db : Session = Depends(get_db), current_user: schema.User = Depends(jwt_bearer)):
    return allBlogs(request, db)


# Get single blog data
@router.get('/blog/{id}', status_code=status.HTTP_202_ACCEPTED, response_model= schema.ShowBlogs)
def blog(id:int, db : Session = Depends(get_db), current_user: schema.User = Depends(jwt_bearer)):
    return singleBlog(id, db)


# Update blog data
@router.put('/update/{id}', status_code=status.HTTP_202_ACCEPTED)
def update_Blog(id:int,request: schema.Blog, db : Session = Depends(get_db), current_user: schema.User = Depends(jwt_bearer)):
    return updateBlog(id, request, db)


# Delete single blog data
@router.delete('/delete/{id}', status_code=status.HTTP_200_OK)
def delete_Blog(id:int, db : Session = Depends(get_db), current_user: schema.User = Depends(jwt_bearer)):
